...
async function main() {
  ...
  const sectors = sectorsRaw.map(s => ({
    id: s.id,
    name: s.name,
    employee_count: s.count ?? s.employeeCount ?? 0,
    avg_salary: s.avgSalary ?? s.medianSalary ?? 0,
    median_salary: s.medianSalary ?? 0,
    min_salary: s.minSalary ?? 0,
    max_salary: s.maxSalary ?? 0,
    total_compensation: s.totalComp ?? s.totalCompensation ?? 0,
  ...
  await upsert('stats_summary', [{
    id: 1,
    total_records: stats.totalRecords ?? stats.totalEmployees ?? 0,
    unique_employers: stats.uniqueEmployers ?? stats.totalEmployers ?? 0,
    latest_year: stats.latestYear ?? 2024,
    median_salary: stats.medianSalary ?? 0,
    total_compensation: stats.totalCompensation ?? stats.totalComp ?? 0,
