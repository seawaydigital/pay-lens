...
- **CI** — `ci.yml` runs lint/typecheck/test/build (Turso secrets); `deploy.yml` deploys to GitHub Pages and runs `CREATE INDEX IF NOT EXISTS idx_disc_title_year_sal` post-build to auto-restore the covering index after any DB reimport. No Python/ETL jobs remain.
- **DB keep-alive** — `.github/workflows/keep-alive.yml` pings Turso every 5 min via cron to prevent cold starts. `SiteHeader` also fires a background `SELECT 1 FROM stats_summary` on mount to warm the DB before users navigate to data-heavy pages.
- **Covering index auto-ensured** — `deploy.yml` runs `CREATE INDEX IF NOT EXISTS idx_disc_title_year_sal ON disclosures(job_title, year, salary_paid)` after each build (no-op when index already exists). Still run `node scripts/add-person-index.mjs` immediately after any manual DB reimport — don't wait for the next deploy.
- **`/person` page** — single `turso.batch()` with 7 SQL subqueries in one HTTP round trip (no two-phase effect pattern). Primary record + salary history + peer percentile + employer rank + sector median + colleagues all fetched together. Known architecture deviation from db.ts pattern; works correctly.
